    ...
    async function handleAddShipment(e: React.FormEvent) {
        ...
        for (const line of validLines) {
            ...
            if (product.stock_quantity < line.quantity) {
                alert(
                    `Недостаточно товара на складе: ${product.name}. Остаток: ${product.stock_quantity} ${product.unit}, требуется: ${line.quantity} ${product.unit}`,
                );
                return;
            }
        }
        ...
        try {
            ...
            const notesSuffix =
                shipmentMode === "order" && selectedOrderId
            ...
            <div className="flex flex-wrap gap-3 justify-between items-start mb-6 sm:mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                    <button
                        ...
                        className="flex items-center space-x-2 bg-red-600 text-white px-4 py-2.5 rounded-lg hover:bg-red-700 transition-colors whitespace-nowrap"
                    >
                ...
                <form onSubmit={handleAddReceiving} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setReceivingForm({ ...receivingForm, quantity: parseInt(e.target.value) || 0 })
                            }
                        ...
                        <div>
                            ...
                            {orders.length === 0 && (
                                <p className="text-xs text-gray-500 mt-1">
                                    Нет заказов со статусами: Доставляется, Подтвержден, В обработке
                                </p>
                            )}
                        </div>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="text-center p-4 bg-yellow-50 rounded-lg">
                        <div className="text-3xl font-bold text-yellow-600">
                            {products
                                .reduce((sum, p) => sum + p.stock_quantity * p.price_per_sqm, 0)
